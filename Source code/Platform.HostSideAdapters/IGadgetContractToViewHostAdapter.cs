...
{
    [HostAdapter]
    public class IGadgetContractToViewHostAdapter : IGadget
    {
        private readonly Contracts.IGadget _contract;
        private ContractHandle _handle;

        public IGadgetContractToViewHostAdapter(Contracts.IGadget contract)
        {
            _contract = contract;
        ...
        {
            get { return _execute ?? (_execute = application => _contract.Execute(IApplicationHostAdapter.ViewToContractAdapter(application))); }
            set
            {
                if (_execute != value)
                {
                    _execute = value;
                    _contract.Execute = application => value(IApplicationHostAdapter.ContractToViewAdapter(application));
                }
            }
        }
