...
{
    public class IGadgetViewToContractHostAdapter : ContractBase, Contracts.IGadget
    {
        private readonly IGadget _view;

        public IGadgetViewToContractHostAdapter(IGadget view)
        {
            _view = view;
        }
        ...
        {
            get { return _execute ?? (_execute = application => _view.Execute(IApplicationHostAdapter.ContractToViewAdapter(application))); }
            set
            {
                if (_execute != value)
                {
                    _execute = value;
                    _view.Execute = application => value(IApplicationHostAdapter.ViewToContractAdapter(application));
                }
            }
        }
